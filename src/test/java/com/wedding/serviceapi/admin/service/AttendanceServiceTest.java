...
class AttendanceServiceTest {
    ...
    void changeAttendanceType() {
        ...
        Users user1 = Users.builder().id(1L).name("user1").build();
        Guests guest1 = Guests.builder().users(user1).attendance(AttendanceType.YES).build();
        doReturn(Optional.of(guest1)).when(guestsRepository).findByUsersIdAndBoardsId(anyLong(), anyLong());
        // when
        attendanceService.changeAttendance(anyLong(), anyLong(), type);
        // then
        assertThat(guest1.getAttendance()).isEqualTo(AttendanceType.NO);
    }
