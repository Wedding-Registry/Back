...
class MemoServiceTest {
    ...
    void init() {
        ...
        UsersGoods usersGoods8 = UsersGoods.builder().users(savedUsers).goods(goods2).boards(savedBoards).updatedUsersGoodsName(goods2.getGoodsName()).updatedUsersGoodsPrice(goods2.getGoodsPrice()).wishGoods(true).build();
    ...
    void sliceTest() {
        ...
        // when
        Slice<UsersGoods> result = memoService.getAllItemWish(savedUsers.getId(), savedBoards.getId(), pageable);
        // then
        log.info("result = {}", result.getContent().size());
